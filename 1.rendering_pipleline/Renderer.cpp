...
// edgetable 에서 preserveVertex를 따로 저장
void Renderer::buildEdgetable(int nFace)
{
	...
	for (int i = 0; i < m_face[nFace].m_nNumVertex; i++)
	{
		for (int j = 0; j < 3; j++)
		{
			//m_tramsformedVertex[m_face[nFace].m_tramsformedVertex[i] - 1][j];
			//m_tramsformedVertex[m_face[nFace].m_tramsformedVertex[(i + 1) % m_face[nFace].m_nNumVertex] - 1][j] = m_vertex[m_face[nFace].m_vertex[(i + 1) % m_face[nFace].m_nNumVertex] - 1][j];
			//vertices[0][j] = m_tramsformedVertex[m_face[nFace].m_tramsformedVertex[i] - 1][j]; //transformed vertex
			//vertices[1][j] = m_tramsformedVertex[m_face[nFace].m_tramsformedVertex[(i + 1) % m_face[nFace].m_nNumVertex] - 1][j]; ////transformed vertex
...
//void Renderer::preserveVertex(Vector4 v)
//{
...
//		v.y = m_vertex[i][1];
//		v.z = m_vertex[i][2];
//		// m_vertex = preserve vertex
//		// preserve vertex *  (matrix * matrix)
//		// preserve vertex *  (matrix * matrix) = m_vertex
//
//	}
//}
...
//void Renderer::currentMatrix()
//{
//	Matrix4 m;
//
//	if (currentMatrixCount == 0)
//	{
//		for (int i = 0; i < 4; i++)
//		{
//			for (int j = 0; j < 4; j++)
...
void Renderer::applyMatrix(Matrix4 m, Matrix4 n)
{
	for (int i = 0; i < m_nNumVertex; i++)
	{
		...
		Vector4 v(m_vertex[i], 1);
		Vector4 w(m_tramsformedVertex[i], 1);
