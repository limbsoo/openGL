...
void Renderer::clearCheckImage()
{
	for (int i = 0; i < checkImageHeight; i++)
	{
		for (int j = 0; j < checkImageWidth; j++)
		{
			...
			//checkImage[i][j][0] = (GLubyte)m_texture[i][j][0];
			//checkImage[i][j][1] = (GLubyte)m_texture[i][j][1];
			//checkImage[i][j][2] = (GLubyte)m_texture[i][j][2];
		}
	}
}
...
void Renderer::applyMatrix(Matrix4 m, Matrix4 n)
{
	...
	for (int i = 0; i < m_nNumVertex; i++)
	{
		...
		Vector4 v1(m_vertexNormal[i], 1);
		//Vector4 v1(m_faceNormal[i], 1);


		v1 = m * v1;

		m_transformedVertexNormal[i][0] = v1.x / v1.w;
		m_transformedVertexNormal[i][1] = v1.y / v1.w;
		m_transformedVertexNormal[i][2] = v1.z / v1.w;

		//m_transformedFaceNormal[i][0] = v1.x / v1.w;
		//m_transformedFaceNormal[i][1] = v1.y / v1.w;
		//m_transformedFaceNormal[i][2] = v1.z / v1.w;
...
Vector4 directionLight(-15.f, -15.f, -15.f);
...
void Renderer::buildEdgetable(int nFace)
{
	...
	for (int i = 0; i < m_face[nFace].m_nNumVertex; i++)
	{
		for (int j = 0; j < 3; j++)
		{
			...
			//nVertices[0][j] = m_transformedFaceNormal[m_face[nFace].m_vertex[i] - 1][j];
			//nVertices[1][j] = m_transformedFaceNormal[m_face[nFace].m_vertex[(i + 1) % m_face[nFace].m_nNumVertex] - 1][j];

			//nVertices[0][j] = m_vertexNormal[m_face[nFace].m_vertex[i] - 1][j];
			//nVertices[1][j] = m_vertexNormal[m_face[nFace].m_vertex[(i + 1) % m_face[nFace].m_nNumVertex] - 1][j];

			nVertices[0][j] = m_transformedVertexNormal[m_face[nFace].m_vertex[i] - 1][j];
			nVertices[1][j] = m_transformedVertexNormal[m_face[nFace].m_vertex[(i + 1) % m_face[nFace].m_nNumVertex] - 1][j];
